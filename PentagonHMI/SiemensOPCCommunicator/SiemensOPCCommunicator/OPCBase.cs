...
{
    //enum CanonicalDataType { ctReal = 5, ctString = 8, ctBoolean = 11 };
    public class OPCBase : CollectionBase, IOPCDataCallback
    {
        #region Variable
        /* Event */
        public delegate void OnUpdateHandler(object sender, OPCItemUpdateEventArgs e);
        public event OnUpdateHandler OnUpdateEvent;

        public delegate void OnWriteHandler(object sender, OPCItemWriteEventArgs e);
        public event OnWriteHandler OnWriteEvent;
        ...
        bool _CheckACK = false;
        private bool CheckACK
        {
            get { return _CheckACK; }
            set { _CheckACK = value; }
        }
        ...
        public OPCItem this[int index]
        {
            get
            {
                return (OPCItem)this.List[index];
            }
            set
            {
        ...
        /// Constructor
        /// </summary>
        /// <param name="Connection">PLC Connection String</param>
        public OPCBase(string Connection)
        {
            this.Connection = Connection;
        }
        /// <summary>
        /// Constructor that required ACK check
        /// </summary>
        /// <param name="Connection">PLC Connection String</param>
        /// <param name="CheckACK">Required to check ACK</param>
        public OPCBase(string Connection,bool CheckACK)
        {
            this.Connection = Connection;
        ...
        #region Methods
        public int IndexOf(OPCItem Item)
        {
            if (Item != null)
            {
        ...
        public void Remove(OPCItem Item)
        {
            this.InnerList.Remove(Item);
        }
        public void Insert(int index, OPCItem Item)
        {
        ...
        public bool Contains(OPCItem Item)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    */
                    ...
                    ItemDeffArray = new OPCITEMDEF[this.List.Count];

                    for (int i = 0; i < this.List.Count; i++)
                    {
                        ItemDeffArray[i].szAccessPath = "";                                 // Accesspath not needed for this sample
                        ItemDeffArray[i].szItemID = Connection + ((OPCItem)this.List[i]).Address;             // Item ID,
                        ...
                        ItemDeffArray[i].vtRequestedDataType = (short)((OPCItem)this.List[i]).Type;

                        //// set (cannonical) datatype
                        //if (((OPCItem)this.List[i]).Address.ToUpper().Contains("REAL"))
                        //{
                        //    ItemDeffArray[i].vtRequestedDataType = (short)Enumeration.CanonicalDataType.ctReal;
                        //}
                        //else if (((OPCItem)this.List[i]).Address.ToUpper().Contains("STRING"))
                        //{
                        //    ItemDeffArray[i].vtRequestedDataType = (short)Enumeration.CanonicalDataType.ctString;
                        //}
                        //else if (((OPCItem)this.List[i]).Address.ToUpper().Contains("X"))
                        //{
                        //    ItemDeffArray[i].vtRequestedDataType = (short)Enumeration.CanonicalDataType.ctBoolean;
                        //}
                        //else
                        //{
                        //    ItemDeffArray[i].vtRequestedDataType = (short)Enumeration.CanonicalDataType.ctUnknown;
                        //}
                    ...
                    {
                        // Add items to group
                        ((IOPCItemMgt)pobjGroup1).AddItems(this.List.Count, ItemDeffArray, out pResults, out pErrors);

                        // Unmarshal to get the server handles out fom the m_pItemResult
                        // after checking the errors
                        int[] errors = new int[this.List.Count];
                        IntPtr pos = pResults;

                        ItemSvrHandleArray = new int[this.List.Count];
                        Marshal.Copy(pErrors, errors, 0, this.List.Count);
            ...
            int[] pErrors)
        {
            ...
            try
            {
                ...
                for (int nCount = 0; nCount < dwCount; nCount++)
                {
                    ...
                    if (pErrors[nCount] == 0)
                    { 
                        ((OPCItem)this.List[phClientItems[nCount] - 1]).OPCValue = pvValues[nCount];
                        ((OPCItem)this.List[phClientItems[nCount] - 1]).Quality = GetQuality(pwQualities[nCount]);
                        if (((OPCItem)this.List[phClientItems[nCount] - 1]).boolACK)
                        {
                            IsCheck = true;
                        }
                    }
                    else
                    {
                        ...
                        OnUpdateEvent(this, new OPCItemUpdateEventArgs(strItemErr, true, null, Connection));
                        //break;
                        return;
                ...
                if (CheckACK && IsCheck)
                {
                    ...
                    {
                        OnUpdateEvent(this, new OPCItemUpdateEventArgs(string.Empty, false, this.List, Connection));
                    }
                }
                else if (!CheckACK)
                {
            ...
            catch (System.Exception exp)
            {
                if (OnUpdateEvent != null)
                {
                    OnUpdateEvent(this, new OPCItemUpdateEventArgs(exp.Message, true, null, Connection));
                }
                //throw exp;
        ...
        {            
            ...
            {
                OnWriteEvent(this, new OPCItemWriteEventArgs(strResult, (pErrors[0] > 0), ((OPCItem)this.List[pClienthandles[0]]).Address, Connection));
            }
        }
        ...
        public void Write(OPCItem Item)
        {
            ...
            values[0] = Item.OPCValue;

            if (pIOPCAsyncIO2 != null)
            {
                try
                {   // Async write                    
                    pIOPCAsyncIO2.Write(1, new int[1] { this.IndexOf(Item) + 1 }, values, nTransactionID + 1, out nCancelid, out pErrors);
